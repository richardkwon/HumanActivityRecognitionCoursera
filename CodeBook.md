...
The original data set was split into training and test sets (70% and 30%) where each partition is also split into three files that contain 1) measurements from the accelerometer and gyroscope 2) activity label and 3) identifier of the person/subjects.  To compose a single tidy data set, both test and training data sets were combined into a single data set.  Following files were used:
